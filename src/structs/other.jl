...
mutable struct Edge{T1, T2} <: AbstractEdge
    child::T1
    parent::T2
...
mutable struct GaussianRandomWalkParameters{T1,T2,T3,T4}
    origin::T1
    ρ::T2
    ω::T3
    λ::T4
    ...
    GaussianRandomWalkParameters(origin) = begin
        #Create struct
        parameters = new{GRWNode, GRWDrift, GRWVolatility, GRWAutoconnectionStrength}()
...
# Create simpler type alias
const GRWNode = GaussianRandomWalkNode{GaussianRandomWalkParameters, GaussianRandomWalkStates, GaussianRandomWalkHistory}
...
struct GRWDrift{T} <: AbstractGRWParameter
    origin::T
    edges::Vector{Union{Edge{GRWDrift, FixedValueNode}, Edge{GRWDrift, GRWNode}}}
    GRWDrift(origin) = new{GaussianRandomWalkParameters{GRWNode, GRWDrift, GRWVolatility, GRWAutoconnectionStrength}}(origin, Union{Edge{GRWDrift, FixedValueNode}, Edge{GRWDrift, GRWNode}}[])
end
#Type for the volatility parameter ω
struct GRWVolatility{T} <: AbstractGRWParameter
    origin::T
    edges::Vector{Union{Edge{GRWVolatility, FixedValueNode}, Edge{GRWVolatility, GRWNode}}}
    GRWVolatility(origin) = new{GaussianRandomWalkParameters{GRWNode, GRWDrift, GRWVolatility, GRWAutoconnectionStrength}}(origin, Union{Edge{GRWVolatility, FixedValueNode}, Edge{GRWVolatility, GRWNode}}[])
end
#Type for the autoconnection strength parameter λ
struct GRWAutoconnectionStrength{T} <: AbstractGRWParameter
    origin::T
    edges::Vector{Edge{GRWAutoconnectionStrength, FixedValueNode}}
    GRWAutoconnectionStrength(origin) = new{GaussianRandomWalkParameters{GRWNode, GRWDrift, GRWVolatility, GRWAutoconnectionStrength}}(origin, Edge{GRWAutoconnectionStrength, FixedValueNode}[])
end

#FIXME: Use incomplete types to make the self-referiantial types work
#FIXME: Decide on relation between GRWNode and GaussianRandomWalkNode - which is in the API ? 
...
#TODO: Use oftype(x, y) to ensure return types
#TODO: Consider using StaticArrays.jl for small arrays