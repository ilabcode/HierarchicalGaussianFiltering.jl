function posterior_trajectory_plot(agent::AgentStruct, chain::Chains, state::String, iterations::Int, input; color = "red", title = "", alpha = 0.1, linewidth=2)
...
function posterior_trajectory_plot(hgf::HGFStruct, chain::Chains, state::String, iterations::Int, input; color = "red", title = "", alpha = 0.1, linewidth=2)
    sampled_pars = describe(chain)[2].nt.parameters
    medians = getfield(describe(chain)[2].nt,Symbol("50.0%"))
    node = split(state, "__", limit =2)[1]
    state_name = split(state, "__", limit =2)[2]
