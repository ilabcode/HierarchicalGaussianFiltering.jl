function get_params(agent::AgentStruct)
    ...
    for node in keys(agent.perception_struct.input_nodes)
        push!(params_list,(node*"_evolution_rate", agent.perception_struct.input_nodes[node].params.evolution_rate))
    ...
    for node in keys(agent.perception_struct.state_nodes)
        push!(params_list,(node*"_evolution_rate", agent.perception_struct.state_nodes[node].params.evolution_rate))
        push!(params_list,(node*"_posterior_mean", agent.perception_struct.state_nodes[node].history.posterior_mean[1]))
        push!(params_list,(node*"_posterior_precision", agent.perception_struct.state_nodes[node].history.posterior_precision[1]))
