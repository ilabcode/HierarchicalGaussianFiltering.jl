...
x1_x2_coupling_strenght = 1.0,action_noise =2., x1_posterior_mean = 1.03, x1_posterior_precision = 1.64 , x2_posterior_precision = 1.82)

params_prior_list = (x1_evolution_rate = TruncatedNormal(10,7 ,0,Inf), x2_evolution_rate = TruncatedNormal(7,3 ,0,Inf), 
...
function fit_model(my_agent,inputs,responses)
    ...
    @model function fit_hgf(y::Vector{Float64})
        ...
        my_agent.perception_struct.state_nodes["x1"].params.evolution_rate = params["omega1"]
        my_agent.perception_struct.state_nodes["x2"].params.evolution_rate = params["omega2"]
